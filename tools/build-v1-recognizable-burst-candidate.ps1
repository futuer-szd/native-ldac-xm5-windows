...
if ($LASTEXITCODE -ne 0 -or $status.Count -ne 0) {
    throw 'Refusing to build the recognizable-burst candidate from dirty Git source.'
}
...
& $cmake --build $build --config $Configuration `
    --target v1_transport_recognizable_worker v1_presence_agent
if ($LASTEXITCODE -ne 0) { throw 'V1 recognizable worker build failed.' }
if ((& git.exe -C $root rev-parse HEAD).Trim() -ne $commit -or
    @(& git.exe -C $root status --porcelain).Count -ne 0) {
    throw 'Git source changed during recognizable-burst build.'
}
$base = Join-Path $root 'artifacts\v1-engine-ready-observer\candidate'
$out = Join-Path $root 'artifacts\v1-recognizable-burst\candidate'
...
$files = @(
    ...
    @{s=Join-Path $build "$Configuration\v1_transport_recognizable_worker.exe"; r='v1_transport_recognizable_worker.exe'},
...
$manifest = [ordered]@{
    manifest_version=1; transport_policy_version=7; source_commit=$commit;
    driver_tree=$driverTree; source_dirty=$false; configuration=$Configuration;
    capabilities=@('verified_policy_v6_faint_audibility_prerequisite',
...
Write-Host "Built V1 recognizable-burst candidate: $out"
