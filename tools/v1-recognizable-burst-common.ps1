...
function Get-V1RecognizableBurstCandidate {
    param([Parameter(Mandatory = $true)][string]$CandidatePath)
    $root = [System.IO.Path]::GetFullPath($CandidatePath)
    $manifestPath = Join-Path $root 'manifest.json'
    if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
        throw "V1 recognizable-burst manifest is missing: $manifestPath"
    }
    ...
    if ([int]$manifest.manifest_version -ne 1 -or
        [int]$manifest.transport_policy_version -ne 7 -or
        ...
        @($manifestPaths | Select-Object -Unique).Count -ne
            $expectedFiles.Count) {
        throw 'The V1 recognizable-burst candidate contract is invalid.'
    }
    foreach ($file in $manifestFiles) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf) -or
            ...
            throw "V1 recognizable-burst file failed its hash check: $($file.path)"
        }
    }
