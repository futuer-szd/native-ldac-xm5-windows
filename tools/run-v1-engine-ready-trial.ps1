...
if (-not (Test-Path -LiteralPath $manifestPath -PathType Leaf)) {
    throw 'The V1 engine-ready observer manifest is missing.'
}
...
if ([int]$manifest.manifest_version -ne 1 -or
    ...
    throw 'The V1 engine-ready observer manifest contract is invalid.'
}
...
$outputRoot = Join-Path $projectRoot `
    'artifacts\v1-engine-ready-observer\trial'
...
$stubPath = Join-Path $candidateRoot 'v1_engine_ready_stub.exe'

Write-Host "V1 engine-ready observer source: $($manifest.source_commit)"
...
Write-Host 'This gate starts one contained no-media stub; PCM read and Bluetooth OPEN execution are disabled.'
...
$passed = $agentExit -eq 0 -and
    ...
    [bool]$state.engine_ready_observer_enabled -and
    [int]$state.render_query_failures -eq 0 -and
    [int]$state.render_started_events -eq 1 -and
    [int]$state.render_stopped_events -eq 1 -and
...
$result = [ordered]@{
    ...
    engine_exit_events = [int]$state.engine_exit_events
...
if (-not $passed) {
    throw "V1 engine-ready observer trial failed. No Bluetooth OPEN was executed. Result: $resultPath"
}

Write-Host 'V1 engine-ready observer trial passed.'
Write-Host 'One contained engine started, became ready, and stopped cleanly.'
Write-Host 'The reducer requested one transport OPEN; execution stayed at zero.'
