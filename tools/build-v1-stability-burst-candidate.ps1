...
if ($LASTEXITCODE -ne 0 -or $status.Count -ne 0) {
    throw 'Refusing to build the stability-burst candidate from dirty Git source.'
}
...
& $cmake --build $build --config $Configuration `
    --target v1_transport_stability_worker v1_presence_agent
if ($LASTEXITCODE -ne 0) { throw 'V1 stability worker build failed.' }
if ((& git.exe -C $root rev-parse HEAD).Trim() -ne $commit -or
    @(& git.exe -C $root status --porcelain).Count -ne 0) {
    throw 'Git source changed during stability-burst build.'
}
$base = Join-Path $root 'artifacts\v1-engine-ready-observer\candidate'
$out = Join-Path $root 'artifacts\v1-stability-burst\candidate'
...
$files = @(
    ...
    @{s=Join-Path $build "$Configuration\v1_transport_stability_worker.exe"; r='v1_transport_stability_worker.exe'},
...
$manifest = [ordered]@{
    manifest_version=1; transport_policy_version=8; source_commit=$commit;
    driver_tree=$driverTree; source_dirty=$false; configuration=$Configuration;
    capabilities=@('verified_policy_v7_recognizable_audio_prerequisite',
        ...
        'audible_PCM_before_Bluetooth_OPEN','unity_post_volume_gain',
        'independent_hard_limiter_0_25','limiter_engagement_telemetry',
...
Write-Host "Built V1 stability-burst candidate: $out"
