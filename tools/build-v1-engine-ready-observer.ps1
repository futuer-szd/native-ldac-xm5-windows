...
if ($LASTEXITCODE -ne 0 -or
    ...
    throw 'Refusing to build the V1 engine-ready observer from a dirty or unknown Git source.'
}
...
& $cmakePath --build $buildRoot --config $Configuration `
    --target v1_presence_agent v1_engine_ready_stub audio_endpoint_probe
if ($LASTEXITCODE -ne 0) {
    throw "V1 engine-ready observer build failed with exit code $LASTEXITCODE."
}
...
if ($LASTEXITCODE -ne 0 -or
    ...
    throw 'The Git source changed during the V1 engine-ready observer build.'
}
...
$files = @(
    ...
    [ordered]@{
        source = Join-Path $buildRoot `
            "$Configuration\v1_engine_ready_stub.exe"
        relative = 'v1_engine_ready_stub.exe'
    },
    [ordered]@{
        source = Join-Path $buildRoot `
...
Write-Host "Built V1 engine-ready observer: $outputRoot"
