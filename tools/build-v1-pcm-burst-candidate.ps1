...
if ($LASTEXITCODE -ne 0 -or $status.Count -ne 0) {
    throw 'Refusing to build the PCM-burst candidate from dirty Git source.'
}
...
& $cmake --build $build --config $Configuration `
    --target v1_transport_pcm_worker v1_presence_agent
if ($LASTEXITCODE -ne 0) { throw 'V1 PCM worker build failed.' }
if ((& git.exe -C $root rev-parse HEAD).Trim() -ne $commit -or
    @(& git.exe -C $root status --porcelain).Count -ne 0) {
    throw 'Git source changed during PCM-burst build.'
}
$base = Join-Path $root 'artifacts\v1-engine-ready-observer\candidate'
$out = Join-Path $root 'artifacts\v1-pcm-burst\candidate'
...
$files = @(
    ...
    @{s=Join-Path $build "$Configuration\v1_transport_pcm_worker.exe"; r='v1_transport_pcm_worker.exe'},
...
$manifest = [ordered]@{
    manifest_version=1; transport_policy_version=5; source_commit=$commit;
    driver_tree=$driverTree; source_dirty=$false; configuration=$Configuration;
    capabilities=@('verified_four_packet_silence_prerequisite',
        'installed_LdacNative_driver_tree_prerequisite',
        'exact_XM5_ACL_generation','render_demand_authorized',
        'audible_PCM_before_Bluetooth_OPEN','maximum_fixed_gain_0_01',
...
Write-Host "Built V1 bounded PCM-burst candidate: $out"
