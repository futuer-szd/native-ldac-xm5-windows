...
& $cmakePath --build $buildRoot --config $Configuration --target avrcp_transport_probe
if ($LASTEXITCODE -ne 0) {
    throw "AVRCP transport probe build failed with exit code $LASTEXITCODE."
}

$sourcePath = Join-Path $buildRoot "$Configuration\avrcp_transport_probe.exe"
if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) {
    throw "AVRCP transport probe output is missing: $sourcePath"
}
New-Item -ItemType Directory -Path $outputRoot -Force | Out-Null
$destinationPath = Join-Path $outputRoot 'avrcp_transport_probe.exe'
Copy-Item -LiteralPath $sourcePath -Destination $destinationPath -Force
$hash = Get-FileHash -LiteralPath $destinationPath -Algorithm SHA256

Write-Host "Built read-only AVRCP transport probe: $destinationPath"
Write-Host "SHA-256: $($hash.Hash)"
Write-Host 'The probe enumerates the interface and optionally attempts CreateFile only; it never sends an IOCTL or data.'
