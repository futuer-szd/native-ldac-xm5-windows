...
DECLARE_INTERFACE_(IAdapterCommon, IUnknown)
{
    ...
    STDMETHOD_(NTSTATUS,        NativeLdacGetLinkState)
    (
        THIS_
        _Out_ PNATIVE_LDAC_LINK_STATE State
    ) PURE;

    STDMETHOD_(NTSTATUS,        NativeLdacSetLinkState)
    (
        THIS_
        _In_ const NATIVE_LDAC_LINK_STATE* State
    ) PURE;

    STDMETHOD_(NTSTATUS,        NativeLdacGetPhysicalPresence)
    (
        THIS_
        _Out_ PNATIVE_LDAC_PRESENCE_STATE State
    ) PURE;

    STDMETHOD_(NTSTATUS,        NativeLdacSetPhysicalPresence)
    (
        THIS_
        _In_ const NATIVE_LDAC_PRESENCE_STATE* State
    ) PURE;

    STDMETHOD_(BOOL,            NativeLdacIsJackConnected)
    (
        THIS
    ) PURE;

#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
    STDMETHOD_(NTSTATUS,        NativeLdacGetDirectPdoSnapshot)
    (
        THIS_
        _Out_ NLD_DIRECT_PDO_CONTROL_SNAPSHOT_V1* Snapshot
    ) PURE;

    STDMETHOD_(NTSTATUS,        NativeLdacGetDirectPdoMediaStatus)
    (
        THIS_
        _Out_ NLD_DIRECT_PDO_MEDIA_STATUS_V1* Status
    ) PURE;

    STDMETHOD_(NTSTATUS,        NativeLdacWriteDirectPdoMedia)
    (
        THIS_
    ...
    STDMETHOD_(NTSTATUS,        NativeLdacRecoverDirectPdo)
    (
        THIS_
    ...
    STDMETHOD_(VOID,            NativeLdacSetDirectPdoKsState)
    (
        THIS_
        _In_ KSSTATE State,
    ...
    STDMETHOD_(VOID,            NativeLdacRegisterJackEventSink)
    (
        THIS_
    ...
    STDMETHOD_(VOID,            NativeLdacUnregisterJackEventSink)
    (
        THIS_
        _In_ PPORTEVENTS PortEvents
    ) PURE;
