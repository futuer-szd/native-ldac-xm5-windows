...
#define NATIVE_LDAC_MEDIA_WATCHDOG_TIMEOUT_MS 3000u
#endif
...
typedef struct _NATIVE_LDAC_LINK_TIMER_CONTEXT
{
    CAdapterCommon* Adapter;
} NATIVE_LDAC_LINK_TIMER_CONTEXT, *PNATIVE_LDAC_LINK_TIMER_CONTEXT;

WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(
    NATIVE_LDAC_LINK_TIMER_CONTEXT,
    NativeLdacGetLinkTimerContext);

EVT_WDF_TIMER EvtNativeLdacLinkTimer;
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
static VOID NativeLdacDirectPdoStateChangedCallback(
    ...
    public CUnknown    
{
    private:
        ...
        KSPIN_LOCK              m_NativeLdacLinkLock;
        NATIVE_LDAC_LINK_STATE  m_NativeLdacLinkState;
        NATIVE_LDAC_PRESENCE_STATE m_NativeLdacPresenceState;
        BOOLEAN                 m_NativeLdacJackConnected;
        PPORTEVENTS             m_NativeLdacJackEventSink;
        ULONG                   m_NativeLdacJackPinId;
        WDFTIMER                m_NativeLdacLinkTimer;
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
        NLD_BTH_PROFILE_CONTEXT m_NativeLdacBthProfile;
        NLD_BTH_SIGNALING_CONTEXT m_NativeLdacBthSignaling;
        NLD_BTH_SIGNALING_CONTEXT m_NativeLdacBthMedia;
        NLD_DIRECT_PDO_ARBITER_CONTEXT m_NativeLdacDirectPdoArbiter;
        NLD_DIRECT_PDO_DIAGNOSTIC_CONTEXT m_NativeLdacDirectPdoDiagnostic;
        NLD_DIRECT_PDO_DISPATCHER_CONTEXT m_NativeLdacDirectPdoDispatcher;
#endif
    ...
    public:
        ...
        DECLARE_STD_UNKNOWN();
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
        CAdapterCommon(PUNKNOWN UnknownOuter)
            : CUnknown(UnknownOuter)
        {
            NldBthProfileInitialize(&m_NativeLdacBthProfile);
            NldBthSignalingInitialize(&m_NativeLdacBthSignaling);
            NldBthSignalingInitialize(&m_NativeLdacBthMedia);
            NldDirectPdoArbiterRuntimeInitialize(
                &m_NativeLdacDirectPdoArbiter);
            NldDirectPdoDiagnosticRuntimeInitialize(
                &m_NativeLdacDirectPdoDiagnostic);
            NldDirectPdoDispatcherInitialize(
                &m_NativeLdacDirectPdoDispatcher);
        }
#else
...
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
        STDMETHODIMP_(NTSTATUS) NativeLdacGetDirectPdoSnapshot
        (
            _Out_ NLD_DIRECT_PDO_CONTROL_SNAPSHOT_V1* Snapshot
        );

        STDMETHODIMP_(NTSTATUS) NativeLdacGetDirectPdoMediaStatus
        (
            _Out_ NLD_DIRECT_PDO_MEDIA_STATUS_V1* Status
        );

        STDMETHODIMP_(NTSTATUS) NativeLdacWriteDirectPdoMedia
        (
        ...
        STDMETHODIMP_(NTSTATUS) NativeLdacRecoverDirectPdo
        (
            _In_ const NLD_DIRECT_PDO_RECOVERY_REQUEST_V1* Request,
            _In_ ULONG BufferSize
        );

        STDMETHODIMP_(VOID) NativeLdacSetDirectPdoKsState
        (
            _In_ KSSTATE State,
        ...
        STDMETHODIMP_(NTSTATUS) NativeLdacGetLinkState
        (
            _Out_ PNATIVE_LDAC_LINK_STATE State
        );

        STDMETHODIMP_(NTSTATUS) NativeLdacSetLinkState
        (
            _In_ const NATIVE_LDAC_LINK_STATE* State
        );

        STDMETHODIMP_(NTSTATUS) NativeLdacGetPhysicalPresence
        (
            _Out_ PNATIVE_LDAC_PRESENCE_STATE State
        );

        STDMETHODIMP_(NTSTATUS) NativeLdacSetPhysicalPresence
        (
            _In_ const NATIVE_LDAC_PRESENCE_STATE* State
        );

        STDMETHODIMP_(BOOL) NativeLdacIsJackConnected();

        STDMETHODIMP_(VOID) NativeLdacRegisterJackEventSink
        (
            _In_ PPORTEVENTS PortEvents,
            _In_ ULONG PinId
        );

        STDMETHODIMP_(VOID) NativeLdacUnregisterJackEventSink
        (
            _In_ PPORTEVENTS PortEvents
        );

        VOID NativeLdacLinkTimerExpired();
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
        VOID NativeLdacDirectPdoStateChanged();
#endif
...
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
static BOOLEAN NativeLdacDirectPdoSnapshotIsJackConnected(
    _In_ const NLD_DIRECT_PDO_DISPATCHER_SNAPSHOT* Snapshot)
{
    // Keep the MMDevice published across a recoverable media-session fault.
    // Removing the endpoint also removes Windows' jack event subscription,
    // which can prevent a later generation-bound recovery from republishing
    // it. PnP stop/offline remains the authoritative unplug boundary.
    return NldDirectPdoShouldPublishEndpoint(
               Snapshot->Started,
...
{
    CAdapterCommon* adapter =
        reinterpret_cast<CAdapterCommon*>(CallbackContext);

    if (adapter != NULL)
    {
        adapter->NativeLdacDirectPdoStateChanged();
    }
}
#endif
...
--*/
{
...
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
    NldDirectPdoDispatcherSetStateCallback(
        &m_NativeLdacDirectPdoDispatcher,
        NULL,
        NULL);
    NldDirectPdoDiagnosticRuntimeStop(
        &m_NativeLdacDirectPdoDiagnostic,
        NLD_BTH_DEFAULT_REQUEST_TIMEOUT_MS);
    NldDirectPdoDispatcherStop(&m_NativeLdacDirectPdoDispatcher,
                               NLD_BTH_DEFAULT_REQUEST_TIMEOUT_MS);
    NldDirectPdoArbiterRuntimeStop(
        &m_NativeLdacDirectPdoArbiter);
    NldBthSignalingStop(&m_NativeLdacBthSignaling,
                        NLD_BTH_DEFAULT_REQUEST_TIMEOUT_MS);
    NldBthSignalingStop(&m_NativeLdacBthMedia,
                        NLD_BTH_DEFAULT_REQUEST_TIMEOUT_MS);
    NldBthProfileStop(&m_NativeLdacBthProfile);
#endif
    ...
    if (m_NativeLdacLinkTimer)
    {
        (void)WdfTimerStop(m_NativeLdacLinkTimer, TRUE);
        m_NativeLdacLinkTimer = NULL;
    }
...
--*/
{
    ...
    m_NativeLdacJackConnected = FALSE;
    m_NativeLdacJackEventSink = NULL;
    m_NativeLdacJackPinId = ULONG_MAX;
    m_NativeLdacLinkTimer = NULL;
    RtlZeroMemory(&m_NativeLdacLinkState, sizeof(m_NativeLdacLinkState));
    m_NativeLdacLinkState.Size = sizeof(m_NativeLdacLinkState);
    m_NativeLdacLinkState.AbiVersion =
        NATIVE_LDAC_LINK_STATE_ABI_VERSION;
    m_NativeLdacLinkState.State = NativeLdacLinkStateDisconnected;
    m_NativeLdacLinkState.Flags = NATIVE_LDAC_LINK_STATE_FLAG_NONE;
    m_NativeLdacLinkState.UpdatedInterruptTime100ns = KeQueryInterruptTime();
    RtlZeroMemory(
        &m_NativeLdacPresenceState,
        sizeof(m_NativeLdacPresenceState));
    m_NativeLdacPresenceState.Size = sizeof(m_NativeLdacPresenceState);
    m_NativeLdacPresenceState.AbiVersion =
        NATIVE_LDAC_PRESENCE_STATE_ABI_VERSION;
    m_NativeLdacPresenceState.State = NativeLdacPresenceAbsent;
    m_NativeLdacPresenceState.Flags =
        NATIVE_LDAC_PRESENCE_STATE_FLAG_NONE;
    m_NativeLdacPresenceState.UpdatedInterruptTime100ns =
        KeQueryInterruptTime();
    KeInitializeSpinLock(&m_NativeLdacLinkLock);
...
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
    ntStatus = NldBthProfileStart(m_pPhysicalDeviceObject,
                                  &m_NativeLdacBthProfile);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("BthPort profile interface query failed, 0x%x",
                      ntStatus)),
        Done);
    ntStatus = NldBthSignalingStart(&m_NativeLdacBthSignaling,
                                    &m_NativeLdacBthProfile,
                                    m_pDeviceObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("BthPort signaling owner start failed, 0x%x",
                      ntStatus)),
        Done);
    ntStatus = NldBthChannelStart(&m_NativeLdacBthMedia,
                                  &m_NativeLdacBthProfile,
                                  m_pDeviceObject,
                                  NLD_BTH_AVDTP_PSM);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("BthPort media owner start failed, 0x%x",
                      ntStatus)),
        Done);
    ...
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("direct-PDO arbiter start failed, 0x%x",
                      ntStatus)),
        Done);
    ntStatus = NldDirectPdoDispatcherStart(
        &m_NativeLdacDirectPdoDispatcher,
        &m_NativeLdacBthSignaling,
        &m_NativeLdacBthMedia,
        &m_NativeLdacDirectPdoArbiter,
        &m_NativeLdacDirectPdoDiagnostic,
        m_pDeviceObject);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("direct-PDO dispatcher start failed, 0x%x",
                      ntStatus)),
        Done);
    NldDirectPdoDispatcherSetStateCallback(
        &m_NativeLdacDirectPdoDispatcher,
        NativeLdacDirectPdoStateChangedCallback,
        this);
    NativeLdacDirectPdoStateChanged();
    ...
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("direct-PDO diagnostic start failed, 0x%x",
                      ntStatus)),
        Done);
#endif

    WDF_TIMER_CONFIG_INIT(&timerConfig, EvtNativeLdacLinkTimer);
    timerConfig.AutomaticSerialization = FALSE;
    WDF_OBJECT_ATTRIBUTES_INIT_CONTEXT_TYPE(
        &timerAttributes,
        NATIVE_LDAC_LINK_TIMER_CONTEXT);
    timerAttributes.ParentObject = m_WdfDevice;
    timerAttributes.ExecutionLevel = WdfExecutionLevelPassive;
    ntStatus = WdfTimerCreate(
        &timerConfig,
        &timerAttributes,
        &m_NativeLdacLinkTimer);
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("WdfTimerCreate failed, 0x%x", ntStatus)),
        Done);
    NativeLdacGetLinkTimerContext(m_NativeLdacLinkTimer)->Adapter = this;
...
//=============================================================================
#pragma code_seg()
static VOID GenerateNativeLdacJackInfoChange
(
    _In_opt_ PPORTEVENTS PortEvents,
    _In_ ULONG PinId
)
{
...
CAdapterCommon::NativeLdacGetLinkState
(
    _Out_ PNATIVE_LDAC_LINK_STATE State
)
{
    ...
    KeAcquireSpinLock(&m_NativeLdacLinkLock, &oldIrql);
    *State = m_NativeLdacLinkState;
    KeReleaseSpinLock(&m_NativeLdacLinkLock, oldIrql);
    return STATUS_SUCCESS;
}
...
)
{
    ...
    if (State == NULL ||
        State->Size != sizeof(NATIVE_LDAC_LINK_STATE) ||
        State->AbiVersion != NATIVE_LDAC_LINK_STATE_ABI_VERSION)
    {
        return STATUS_REVISION_MISMATCH;
    }
    if (State->Flags != NATIVE_LDAC_LINK_STATE_FLAG_NONE ||
    ...
    m_NativeLdacLinkState.AbiVersion =
        NATIVE_LDAC_LINK_STATE_ABI_VERSION;
    m_NativeLdacLinkState.State = State->State;
    m_NativeLdacLinkState.Flags = NATIVE_LDAC_LINK_STATE_FLAG_NONE;
    m_NativeLdacLinkState.SessionId = State->SessionId;
    m_NativeLdacLinkState.UpdateSequence++;
    m_NativeLdacLinkState.UpdatedInterruptTime100ns = now;
    KeReleaseSpinLock(&m_NativeLdacLinkLock, oldIrql);

    if (m_NativeLdacLinkTimer != NULL)
    {
        (void)WdfTimerStart(
            m_NativeLdacLinkTimer,
...
)
{
    ...
    KeAcquireSpinLock(&m_NativeLdacLinkLock, &oldIrql);
    *State = m_NativeLdacPresenceState;
    KeReleaseSpinLock(&m_NativeLdacLinkLock, oldIrql);
    return STATUS_SUCCESS;
}
...
#pragma code_seg()
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::NativeLdacSetPhysicalPresence
(
    _In_ const NATIVE_LDAC_PRESENCE_STATE* State
)
{
    ...
    BOOLEAN connected;
    BOOLEAN notify;
    PPORTEVENTS eventSink = NULL;
    ULONG eventPinId = ULONG_MAX;
    ULONGLONG now;

    if (State == NULL ||
        State->Size != sizeof(NATIVE_LDAC_PRESENCE_STATE) ||
        State->AbiVersion != NATIVE_LDAC_PRESENCE_STATE_ABI_VERSION)
    {
        return STATUS_REVISION_MISMATCH;
    }
    if (State->Flags != NATIVE_LDAC_PRESENCE_STATE_FLAG_NONE ||
    ...
    now = KeQueryInterruptTime();
    connected = State->State == NativeLdacPresencePresent;
    KeAcquireSpinLock(&m_NativeLdacLinkLock, &oldIrql);
    m_NativeLdacPresenceState.Size = sizeof(m_NativeLdacPresenceState);
    m_NativeLdacPresenceState.AbiVersion =
        NATIVE_LDAC_PRESENCE_STATE_ABI_VERSION;
    m_NativeLdacPresenceState.State = State->State;
    m_NativeLdacPresenceState.Flags =
        NATIVE_LDAC_PRESENCE_STATE_FLAG_NONE;
    m_NativeLdacPresenceState.PresenceGeneration =
        State->PresenceGeneration;
    m_NativeLdacPresenceState.UpdateSequence++;
    m_NativeLdacPresenceState.UpdatedInterruptTime100ns = now;
    notify = connected != m_NativeLdacJackConnected;
    m_NativeLdacJackConnected = connected;
    if (notify && m_NativeLdacJackEventSink != NULL)
    {
        eventSink = m_NativeLdacJackEventSink;
        eventSink->AddRef();
        eventPinId = m_NativeLdacJackPinId;
    }
    KeReleaseSpinLock(&m_NativeLdacLinkLock, oldIrql);
    ...
    GenerateNativeLdacJackInfoChange(eventSink, eventPinId);
    return STATUS_SUCCESS;
}
...
#pragma code_seg()
STDMETHODIMP_(BOOL)
CAdapterCommon::NativeLdacIsJackConnected()
{
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
    NLD_DIRECT_PDO_DISPATCHER_SNAPSHOT snapshot;

    NldDirectPdoDispatcherGetSnapshot(
        &m_NativeLdacDirectPdoDispatcher,
        &snapshot);
    return NativeLdacDirectPdoSnapshotIsJackConnected(&snapshot)
        ? TRUE
        : FALSE;
#else
    KIRQL oldIrql;
    ...
    KeAcquireSpinLock(&m_NativeLdacLinkLock, &oldIrql);
    connected = NativeLdacPresenceStateIsFreshPresent(
        &m_NativeLdacPresenceState,
        now) ? TRUE : FALSE;
    KeReleaseSpinLock(&m_NativeLdacLinkLock, oldIrql);
    return connected;
...
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
VOID CAdapterCommon::NativeLdacDirectPdoStateChanged()
{
    NLD_DIRECT_PDO_DISPATCHER_SNAPSHOT snapshot;
    KIRQL oldIrql;
    BOOLEAN connected;
    ...
    NldDirectPdoDispatcherGetSnapshot(
        &m_NativeLdacDirectPdoDispatcher,
        &snapshot);
    connected = NativeLdacDirectPdoSnapshotIsJackConnected(&snapshot);
...
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::NativeLdacGetDirectPdoSnapshot
(
    _Out_ NLD_DIRECT_PDO_CONTROL_SNAPSHOT_V1* Snapshot
)
{
    ...
    NldDirectPdoControlBuildSnapshot(
        &m_NativeLdacDirectPdoDiagnostic,
        &m_NativeLdacDirectPdoArbiter,
        Snapshot);
    return STATUS_SUCCESS;
...
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::NativeLdacGetDirectPdoMediaStatus
(
    _Out_ NLD_DIRECT_PDO_MEDIA_STATUS_V1* Status
)
{
    ...
    NldDirectPdoDispatcherGetMediaStatus(
        &m_NativeLdacDirectPdoDispatcher,
        Status);
    if (Status->State == NldDirectPdoMediaStreaming &&
        m_NativeLdacLinkTimer != NULL)
    {
        ULONG delayMs = NldDirectPdoDispatcherScheduleMediaWatchdog(
            &m_NativeLdacDirectPdoDispatcher,
            NATIVE_LDAC_MEDIA_WATCHDOG_TIMEOUT_MS);
        if (delayMs != 0u)
        {
            (void)WdfTimerStart(
                m_NativeLdacLinkTimer,
                WDF_REL_TIMEOUT_IN_MS(delayMs));
        }
    }
    return STATUS_SUCCESS;
}
...
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::NativeLdacWriteDirectPdoMedia
(
    _In_reads_bytes_(BufferSize)
        const NLD_DIRECT_PDO_MEDIA_PACKET_V1* Packet,
    _In_ ULONG BufferSize
)
{
    ...
    NldDirectPdoDispatcherGetMediaStatus(
        &m_NativeLdacDirectPdoDispatcher,
        &status);
    if (NldDirectPdoMediaValidatePacket(Packet, BufferSize, &status) !=
        NldDirectPdoMediaValidationOk)
    {
        return STATUS_INVALID_DEVICE_STATE;
    }
    writeStatus = NldDirectPdoDispatcherWriteMedia(
        &m_NativeLdacDirectPdoDispatcher,
        Packet->MediaGeneration,
        (const UCHAR*)Packet + NLD_DIRECT_PDO_MEDIA_PACKET_HEADER_SIZE,
        Packet->PayloadLength);
    if (NT_SUCCESS(writeStatus) && m_NativeLdacLinkTimer != NULL)
    {
        ULONG delayMs = NldDirectPdoDispatcherScheduleMediaWatchdog(
            &m_NativeLdacDirectPdoDispatcher,
            NATIVE_LDAC_MEDIA_WATCHDOG_TIMEOUT_MS);
...
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::NativeLdacRecoverDirectPdo
(
    _In_ const NLD_DIRECT_PDO_RECOVERY_REQUEST_V1* Request,
    _In_ ULONG BufferSize
)
{
    ...
    return NldDirectPdoDispatcherRequestRecovery(
        &m_NativeLdacDirectPdoDispatcher,
        Request,
        BufferSize);
}
...
STDMETHODIMP_(VOID)
CAdapterCommon::NativeLdacSetDirectPdoKsState
(
    _In_ KSSTATE State,
    _In_ ULONG SampleRateHz
)
{
    ...
    if (SampleRateHz != 0u)
    {
        (void)NldDirectPdoDispatcherSetFormat(
            &m_NativeLdacDirectPdoDispatcher,
            SampleRateHz);
    }
    (void)NldDirectPdoDispatcherSetIntent(
        &m_NativeLdacDirectPdoDispatcher,
        intent);
    if (intent != NldDirectPdoKsRunning &&
        m_NativeLdacLinkTimer != NULL)
    {
        (void)WdfTimerStop(m_NativeLdacLinkTimer, FALSE);
    }
}
#endif
...
#pragma code_seg()
STDMETHODIMP_(VOID)
CAdapterCommon::NativeLdacRegisterJackEventSink
(
    _In_ PPORTEVENTS PortEvents,
    _In_ ULONG PinId
)
{
    ...
    KeAcquireSpinLock(&m_NativeLdacLinkLock, &oldIrql);
    if (m_NativeLdacJackEventSink == NULL ||
        m_NativeLdacJackEventSink == PortEvents)
    {
        m_NativeLdacJackEventSink = PortEvents;
        m_NativeLdacJackPinId = PinId;
    }
    KeReleaseSpinLock(&m_NativeLdacLinkLock, oldIrql);
}
...
#pragma code_seg()
STDMETHODIMP_(VOID)
CAdapterCommon::NativeLdacUnregisterJackEventSink
(
    _In_ PPORTEVENTS PortEvents
)
{
    ...
    KeAcquireSpinLock(&m_NativeLdacLinkLock, &oldIrql);
    if (m_NativeLdacJackEventSink == PortEvents)
    {
        m_NativeLdacJackEventSink = NULL;
        m_NativeLdacJackPinId = ULONG_MAX;
    }
...
//=============================================================================
#pragma code_seg()
VOID CAdapterCommon::NativeLdacLinkTimerExpired()
{
#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
    ULONG delayMs = NldDirectPdoDispatcherCheckMediaWatchdog(
        &m_NativeLdacDirectPdoDispatcher,
        NATIVE_LDAC_MEDIA_WATCHDOG_TIMEOUT_MS);

    if (delayMs != 0u && m_NativeLdacLinkTimer != NULL)
    {
        (void)WdfTimerStart(
            m_NativeLdacLinkTimer,
            WDF_REL_TIMEOUT_IN_MS(delayMs));
...
#else
    KIRQL oldIrql;
    ULONGLONG linkRemaining = 0;
    ULONGLONG presenceRemaining = 0;
    ULONGLONG nextRemaining = 0;
    ULONGLONG now = KeQueryInterruptTime();
    PPORTEVENTS eventSink = NULL;
    ULONG eventPinId = ULONG_MAX;

    KeAcquireSpinLock(&m_NativeLdacLinkLock, &oldIrql);
    if (m_NativeLdacLinkState.State == NativeLdacLinkStateConnected)
    {
        linkRemaining = NativeLdacLinkStateRemainingFreshTime100ns(
            &m_NativeLdacLinkState,
            now);
        if (linkRemaining == 0)
        {
            (void)NativeLdacExpireConnectedLinkState(
                &m_NativeLdacLinkState,
                now);
        }
    }
    if (m_NativeLdacPresenceState.State == NativeLdacPresencePresent)
    {
        presenceRemaining = NativeLdacPresenceStateRemainingTime100ns(
            &m_NativeLdacPresenceState,
            now);
        if (presenceRemaining == 0)
        {
            (void)NativeLdacExpirePresenceState(
                &m_NativeLdacPresenceState,
                now);
            if (m_NativeLdacJackConnected &&
                m_NativeLdacJackEventSink != NULL)
            {
                eventSink = m_NativeLdacJackEventSink;
                eventSink->AddRef();
    ...
    if (linkRemaining != 0)
    {
        nextRemaining = linkRemaining;
    }
    if (presenceRemaining != 0 &&
        (nextRemaining == 0 || presenceRemaining < nextRemaining))
    {
        nextRemaining = presenceRemaining;
    }
    KeReleaseSpinLock(&m_NativeLdacLinkLock, oldIrql);

    if (nextRemaining != 0 && m_NativeLdacLinkTimer != NULL)
    {
        ULONG delayMs = (ULONG)((nextRemaining + 9999ull) / 10000ull);
        (void)WdfTimerStart(
            m_NativeLdacLinkTimer,
            WDF_REL_TIMEOUT_IN_MS(max(delayMs, 1u)));
    }
    GenerateNativeLdacJackInfoChange(eventSink, eventPinId);
#endif
}
...
//=============================================================================
#pragma code_seg()
VOID EvtNativeLdacLinkTimer
(
    _In_ WDFTIMER Timer
)
{
    PNATIVE_LDAC_LINK_TIMER_CONTEXT context =
        NativeLdacGetLinkTimerContext(Timer);

    if (context != NULL && context->Adapter != NULL)
    {
        context->Adapter->NativeLdacLinkTimerExpired();
    }
}
