...
--*/
{
    ...
    if (m_DeviceType == eSpeakerDevice &&
        m_AdapterCommon != NULL &&
        m_PortEvents != NULL)
    {
        m_AdapterCommon->NativeLdacUnregisterJackEventSink(m_PortEvents);
    }
} // ~CMiniportTopology
...
--*/
{
    ...
    if (m_DeviceType == eSpeakerDevice)
    {
        m_AdapterCommon->NativeLdacRegisterJackEventSink(
            m_PortEvents,
...
#pragma code_seg()
NTSTATUS
CMiniportTopology::EventHandlerJackInfoChange
(
    _In_ PPCEVENT_REQUEST EventRequest
)
{
    if (EventRequest == NULL ||
        EventRequest->EventItem == NULL ||
        EventRequest->EventItem->Set == NULL ||
        !IsEqualGUIDAligned(
            *EventRequest->EventItem->Set,
            KSEVENTSETID_PinCapsChange) ||
        EventRequest->EventItem->Id != KSEVENT_PINCAPS_JACKINFOCHANGE)
    {
    ...
    case PCEVENT_VERB_ADD:
        if (EventRequest->EventEntry == NULL || m_PortEvents == NULL)
        {
            return STATUS_UNSUCCESSFUL;
        }
        AddEventToEventList(EventRequest->EventEntry);
        // The LDAC link may already be connected before AudioEndpointBuilder
        // subscribes. Replay the current jack state so that the first
        // connection cannot be lost between the state update and event add.
        GenerateJackInfoChange();
        return STATUS_SUCCESS;
...
#pragma code_seg()
VOID
CMiniportTopology::GenerateJackInfoChange()
{
...
#pragma code_seg()
NTSTATUS
CMiniportTopology::EventHandlerControlChange
(
    _In_ PPCEVENT_REQUEST EventRequest
)
{
    if (EventRequest == NULL ||
        ...
        EventRequest->EventItem->Id != KSEVENT_CONTROL_CHANGE)
    {
        return STATUS_INVALID_PARAMETER;
    }
...
#pragma code_seg()
VOID
CMiniportTopology::GenerateControlChange
(
    _In_ ULONG NodeId
)
{
    GUID eventSet = KSEVENTSETID_AudioControlChange;
...
#pragma code_seg()
BOOL
CMiniportTopology::NativeLdacIsJackConnected()
{
    return m_AdapterCommon != NULL
        ? m_AdapterCommon->NativeLdacIsJackConnected()
        : FALSE;
}
...
#pragma code_seg("PAGE")
NTSTATUS
CMiniportTopology::PropertyHandlerJackContainerId
(
    _In_        PPCPROPERTY_REQUEST                         PropertyRequest,
    _In_        ULONG                                       cJackDescriptions,
    _In_reads_(cJackDescriptions) PKSJACK_DESCRIPTION       *JackDescriptions,
    _In_        const GUID                                  *ContainerId
)
{
    PAGED_CODE();

    if (PropertyRequest == NULL ||
        JackDescriptions == NULL ||
        ContainerId == NULL ||
    ...
    if (pinId >= cJackDescriptions || JackDescriptions[pinId] == NULL)
    {
        return STATUS_INVALID_DEVICE_REQUEST;
    }

    if ((PropertyRequest->Verb & KSPROPERTY_TYPE_BASICSUPPORT) != 0)
    {
    ...
    RtlCopyMemory(PropertyRequest->Value, ContainerId, sizeof(GUID));
    PropertyRequest->ValueSize = sizeof(GUID);
    return STATUS_SUCCESS;
}
