...
)
{
    ...
    switch (State_)
    {
        ...
        case KSSTATE_RUN:
            ...
            {
                m_pMiniport->BeginPcmStream(
                    m_pWfExt->Format.nSamplesPerSec,
                    m_pWfExt->Format.nChannels,
                    m_pWfExt->Samples.wValidBitsPerSample,
    ...
    if (!m_bCapture && m_KsState == KSSTATE_RUN && State_ != KSSTATE_RUN)
    {
        m_pMiniport->EndPcmStream();
    }

#ifdef NATIVE_LDAC_DIRECT_PDO_PROTOTYPE
    if (!m_bCapture)
    {
        m_pMiniport->GetAdapterCommObj()->NativeLdacSetDirectPdoKsState(
            State_,
            m_pWfExt != NULL ? m_pWfExt->Format.nSamplesPerSec : 0u);
    }
#endif
...
)
{
    ...
    else
    {
        ...
        // Read from the WaveRT buffer into the bounded user-mode PCM channel.
...
--*/
{
    ...
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
        m_pMiniport->PushPcm(m_pDmaBuffer + bufferOffset, runWrite);
